...
echo "Running itkpixv2 Column Test 5 Times" 
timeout 100 ./bin/Itkpixv2ColumnTest -r $controllerCfg -c $connectivityCfg -n 5 -w -e 1
for i in {1..5}
do
    echo "Running corecolumnscan" $i
    timeout 300 ./bin/scanConsole -r $controllerCfg -c $connectivityCfg -s configs/scans/itkpixv2/corecolumnscan.json 
done
