...
void NetioTxCore::prepareTrigger(){
  for(auto it : m_elinks){
    ...
    if(m_pixFwTrigger){ //special 16b character in the F/W = {1110, #iteration (7b), frequency(5b)
      ...
      if(trigFreq_ratio > 31) { nlog->error("m_trigFreq {} Hz not supported by the F/W. Supported frequency is >= 9.8 kHz. Setting trigFreq_ratio=31 corresponding to the lowest supported trigger frequency (9.8 kHz) and proceeding with the scan.", m_trigFreq); trigFreq_ratio = 31;}
      if(trigFreq_ratio <= 0) { nlog->error("m_trigFreq {} Hz not supported by the F/W. Supported frequency is <~ 156 kHz. Setting trigFreq_ratio=1 corresponding to the highest supported trigger frequency (156 kHz) and proceeding with the scan.", m_trigFreq); trigFreq_ratio = 1;}
      if(m_trigCnt > 127)     { nlog->error("m_trigCnt {} not supported by the F/W. Supported range is 1 to 127. Setting injections = 127 and proceeding with the scan.", m_trigCnt); m_trigCnt = 127;}
      ...
      static int count=0;
      if(count==0){
        nlog->info("Firmware-trigger frequency {} Hz as used by FELIX, given trigger frequency {} Hz in YARR scan config.", trigFreq_FW, m_trigFreq);
        count++;
      }
