...
void NetioTxCore::prepareTrigger(){
  for(auto it : m_elinks){
    ...
    if(m_pixFwTrigger){ //special 16b character in the F/W = {1110, #iteration (7b), frequency(5b)
      ...
      if(trigFreq_ratio > 31) {cerr<<"m_trigFreq "<<m_trigFreq<<" Hz not supported by the F/W. Supported frequency is >= 9.8 kHz"<<endl; exit(1);}
      if(trigFreq_ratio == 0) {cerr<<"m_trigFreq "<<m_trigFreq<<" Hz not supported by the F/W. Supported frequency is <~ 156 kHz"<<endl; exit(1);}
      if(m_trigCnt > 127)     {cerr<<"m_trigCnt "<<m_trigCnt<<" Hz not supported by the F/W. Supported range is 1 to 127"<<endl; exit(1);}
