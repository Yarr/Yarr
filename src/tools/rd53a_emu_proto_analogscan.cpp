...
int main(int argc, char *argv[]) {
    ...
    if( argc > 2 ) {
        fe.writeRegister(&Rd53a::InjVcalHigh, atoi( argv[1] ) );
    }
    if( argc > 3 ) {
        fe.writeRegister(&Rd53a::InjVcalHigh, atoi( argv[2] ) );
    }
    ...
    emuCtrl.setTrigWord(&trigWord[0], 16);
