...
bool testDataPacketsStatic(HwController& hwCtrl, bool do_spec_specific) {
  ...
  logger->debug(" Set MaskInput7 to 0xfffe0000");
...
bool testDataPacketsPulse(HwController& hwCtrl, bool do_spec_specific) {
  ...
  // Set some mask registers
  logger->debug(" Set MaskInput3 to 0x0000ffff");
  sendCommand(star.write_abc_register(19, 0x0000ffff), hwCtrl); // MaskInput3
  logger->debug(" Set MaskInput7 to 0x00ffff00");
  sendCommand(star.write_abc_register(23, 0x00ffff00), hwCtrl); // MaskInput7
