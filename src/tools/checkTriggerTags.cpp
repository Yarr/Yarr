...
int main(int argc, char *argv[]) {
    ...
    bool verbose = false;
    while ((c = getopt(argc, argv, "hvt:s:e:")) != -1) {
		switch (c) {
		case 'h':
		    usage(argv);
		    return 0;
    ...
    for(int n=1; n<argc; n++) {
        ...
        while(inputFile) {
            ...
            if (m_curBlock == n_trig) {
                if(verbose) std::cout << "Sending event .. "<< eventCnt << std::endl;
                eventCnt++;
                m_curBlock = 0;
            }
    ...
    if(verbose) std::cout << "Done, bye!" << std::endl;
    return 0;
}
