...
auto logger = logging::make_log("rd53bReadRegister");

/* To be updated to decode pixel registers */
void decode(RawData *data, Rd53b &rd53b, std::queue<std::pair<unsigned, unsigned> > &address)
{
...
int main(int argc, char *argv[])
{
	...
	logger->info("\033[1;31m# RD53B Test Tool: self trigger #\033[0m");
	logger->info("\033[1;31m#################################\033[0m");
    ...
    if (initialize)
    {
        ...
        /* If randomization of pixel registers is needed, do it now */
        if (randomize)
        {
	...
	{
		/* Is "=" implemented for the class? If not, start from default values */
		unsigned counter = 0;
			...
			for (unsigned row = 0; row < Rd53b::n_Row; row++)
			{
				...
				if (counter >= counter_max)
				{
					...
					do
					{
						...
						if (data != NULL)
						{
							decode(data, rd53b_readback[idx], address);
						}
					} while (data != NULL);
