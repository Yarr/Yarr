...
int main(int argc, char *argv[]) { //./plotWithRoot_NoiseMap path/to/directory file_ext goodDiff_On
	...
	while ((dirp = readdir(dp))) { //pointer to structure representing directory entry at current position in directory stream, and positions directory stream at the next entry. Returns a null pointer at the end of the directory stream.
		...
		if ( strstr( file_path, "NoiseMap.dat") != NULL) { //if filename contains string declared in argument.
			...
			//Plot RMS Range histograms for Synchronous, Linear, and Differential FEs.	
			for (int i=0; i<3; i++) {
				...
				range_rms[i]->SetMaximum((range_rms[i]->GetMaximum())*1.25);
				rmsrange_c[i]->Update();
			...
			hs_rmsrange->GetXaxis()->SetLabelSize(0.065);
			hs_rmsrange->GetYaxis()->SetLabelSize(0.045);
			gPad->SetLogy(0);
			c_Stackrms->Modified();
			gStyle->SetOptStat(0);
			...
			hs_rmsrange->GetXaxis()->SetLabelSize(0.05);
			hs_rmsrange->GetYaxis()->SetLabelSize(0.03);
			
			tname->DrawLatex(0.28,0.96, rd53.c_str());
			tname->DrawLatex(0.8, 0.96, chipnum.c_str());
			
			hs_rmsrange->SetMaximum((hs_rmsrange->GetMaximum())*1.21);
			c_Stackrms->Update();
			...
			for (int i=0; i<3; i++) {
				...
				range_gaus[i]->SetMaximum((range_gaus[i]->GetMaximum())*1.25);
			...
			hs_gausrange->GetXaxis()->SetLabelSize(0.065);
			hs_gausrange->GetYaxis()->SetLabelSize(0.045);
			gPad->SetLogy(0);
			...
			hs_gausrange->GetXaxis()->SetLabelSize(0.05);
			hs_gausrange->GetYaxis()->SetLabelSize(0.03);
			
			tname->DrawLatex(0.28,0.96, rd53.c_str());
			tname->DrawLatex(0.8, 0.96, chipnum.c_str());
			
			hs_gausrange->SetMaximum((hs_gausrange->GetMaximum())*1.21);
			c_Stackgaus->Update();
