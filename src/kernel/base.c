...
static int __devinit pcidriver_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	else if ((id->vendor == PCIXROB_VENDOR_ID) &&
		(id->device == PCIXROB_DEVICE_ID))
	{
		/* It is a PCI-X ROBIN" board */
		mod_info( "Found PCI-X ROBIN2 board at %s\n", dev_name(&pdev->dev));
	}
	else if ((id->vendor == PCIEROB_VENDOR_ID) &&
		(id->device == PCIEROB_DEVICE_ID))
	{
		/* It is a PCI-e ROBIN" board */
		mod_info( "Found PCIe ROBIN2 board at %s\n", dev_name(&pdev->dev));
	}
	...
	else if ((id->vendor == PCIE_SPEC_VENDOR_ID) &&
		(id->device == PCIE_SPEC_DEVICE_ID))
	{
        /* It is a SPEC card */
		mod_info("Found SPEC card at %s\n", dev_name(&pdev->dev));
		pci_set_master(pdev); // Need bus master for DMA SG
	}
	else
	{
