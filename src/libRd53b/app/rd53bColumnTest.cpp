...
// # Project: Yarr
// # Description: RD53B Core Column Issue Testing
// # Date: January 2024
// ############################
...
auto logger = logging::make_log("rd53bColumnTest");
...
void clear_and_flush(std::unique_ptr<HwController>& hwCtrl, int msec){
  hwCtrl->writeFifo(0xAAAA0000 |Rd53bCmd::genClear(16)[0]);
...
int main (int argc, char *argv[]) {
    ...
    j["log_config"][2]["name"] = "Rd53bGlobalCfg";
	...
	continue;
      ...
      auto rd53b_fe = init_fe(hwCtrl, conn, ichip);
      Rd53b *rd53b = dynamic_cast<Rd53b*>(rd53b_fe.get());
      auto feCfg = dynamic_cast<FrontEndCfg*>(rd53b_fe.get());
      ...
      unsigned nCol = Rd53b::n_Col;
      unsigned nRow = Rd53b::n_Row;
	...
	for (unsigned row =0; row<nRow; row++){
	  rd53b->setEn(col,row,turnOnPixels);
	  rd53b->setInjEn(col,row,turnOnPixels);
	  rd53b->setHitbus(col,row,turnOnPixels);
	  allPixels.push_back(std::make_pair(col,row));
	}
      }
      rd53b->configurePixels(allPixels);
      //logger->info("Done");
	...
	for (int ivar = 0; ivar<numEnCoreColsVars; ivar++){
	  ...
	  rd53b->writeNamedRegister(colName,0);
	}
	...
	uint32_t efuse_data_raw_orig = rd53b->readEfusesRaw();
	...
	for (int ivar = 0; ivar<numEnCoreColsVars; ivar++){
	  ...
	  for (int icol = 0; icol<numCols; icol++){
	    ...
	    uint16_t init_ptot;
	    stat = rd53b->readNamedRegister(colNamePtot, init_ptot);
	    stat = rd53b->writeNamedRegister(colName,valToSet);

	    logger->debug("Set {} from chip {} to {}",colName,ichip,valToSet);
	    stat = rd53b->writeNamedRegister(colNameRes,valToSet);
	    stat = rd53b->writeNamedRegister(colNameCal,valToSet);
	    stat = rd53b->writeNamedRegister(colNameHitOr,valHitOr);

	    logger->info("Set {} from chip {} to {}",colName,ichip,valToSet);

	    //Read Efuses
	    uint32_t efuse_data_raw = rd53b->readEfusesRaw();
	    ...
	    if (good_result){
	      ...
	      auto trigger1 = (uint32_t)Rd53bCmd::genTrigger(0xF,0)[0] << 16;
	      auto trigger2 = Rd53bCmd::genTrigger(0xF,1)[0];
	    ...
	    //Reset
	    stat = rd53b->writeNamedRegister(colName,0);
	    stat = rd53b->writeNamedRegister(colNameRes,0);
      stat = rd53b->writeNamedRegister(colNameCal,0);
	    stat = rd53b->writeNamedRegister(colNameHitOr,(int)pow(2, numCols)-1);
	    hwCtrl->writeFifo(0xAAAA0000 | Rd53bCmd::genClear(16)[0]);
	    std::this_thread::sleep_for(std::chrono::milliseconds(200));
	    hwCtrl->writeFifo(0xAAAA0000 | Rd53bCmd::genClear(16)[0]);
	    std::this_thread::sleep_for(std::chrono::milliseconds(100));
