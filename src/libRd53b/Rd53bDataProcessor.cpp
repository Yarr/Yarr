...
uint64_t Rd53bDataProcessor::retrieve(const unsigned length, const bool checkEOS, const bool skipNSCheck)
{
    ...
    if (unlikely(_blockIdx >= _NB))
        logger->error("Data error: end of current raw data container reached while still reading stream!");
    ...
    {                
        ...
        if (_blockIdx-- <= _procRawDataSize)
            _procRawDataSize -= (_curInV->words[--_rawDataIdx] >> 1);
        return 0;
    }
    ...
    else
    {
        ...
        if (unlikely(_blockIdx >= _NB))
        {
            ...
            if (_bitIdx + length > BLOCKSIZE && !skipNSCheck)
                logger->error("Data error: end of current raw container reached while still reading stream!");
...
void Rd53bDataProcessor::rollBack(const unsigned length)
{
    ...
    {                              
        ...
        _data = &_curInV->buf[_rawDataIdx][(--_blockIdx - _procRawDataSize) << 1]; // Also roll back the block index and data word pointer
    }
}
...
void Rd53bDataProcessor::process_core()
{
    ...
    while (!m_input->empty())
    {
        ...
        {
            curOut[activeChannels[i]].reset(new FrontEndData(_curInV->stat));
        ...
        if (unlikely(!(_data[0] >> 31 & 0x1)))
            logger->error("Expect new stream while NS = 0 at the beginning of the raw data container");
...
bool Rd53bDataProcessor::getNextDataBlock()
{
    ...
    if ((_blockIdx - _procRawDataSize) >= (_curInV->words[_rawDataIdx] >> 1))
    {
        _procRawDataSize += (_curInV->words[_rawDataIdx] >> 1); // Increase the size of processed raw data
        _rawDataIdx++;
    }

    // Upate the data pointer. Note the meaning of block index is the first block that is *unprocessed*
    _data = &_curInV->buf[_rawDataIdx][(_blockIdx - _procRawDataSize) << 1];

    // Bit index will be handled in retrieve() method
    // _bitIdx = 1; // Reset bit index within the block. Note the value starts from 1 as we assume the NS bit has been read
