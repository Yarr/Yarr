...
bool Rd53bMaskLoop::getNeighboursMap(int col, int row,int sensorType, int maskSize,  std::vector<std::pair<int, int>>  &neighboursindex){
    ...
    for ( auto& p : AllNeighboursCoordinates[sensor][col%2]){
        ...
        if (static_cast<unsigned int>(col+shift_col) >(Rd53b::n_Col-1) || static_cast<unsigned int>(row+shift_row) >(Rd53b::n_Row-1)  ) {i++; continue;}
        if (static_cast<unsigned int>(col+shift_col) >(Rd53b::n_Col-1) || static_cast<unsigned int>(row+shift_row) >(Rd53b::n_Row-1)  ) {i++; continue;}
...
bool Rd53bMaskLoop::ignorePixel(int col, int row){
    ...
    if (m_sensorType!=SquareSensor){
        ...
        if (m_sensorType==RecSensorUpDown)
            if ( ( (col%2)==0 and row==0) or ( (col%2==1 and row==Rd53b::n_Row-1))) return true;

        if (m_sensorType==RecSensorDownUp)
            if ( ( (col%2)==1 and row==0) or ( (col%2==0 and row==Rd53b::n_Row-1))) return true;
    }
