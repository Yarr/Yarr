...
EmuShm::EmuShm(key_t key, uint32_t size, bool create)
{
    ...
    if (create)
    {
        ...
        memcpy(&shm_pointer[index_of_write_index], &write_index, 4);	// write the write_index value to the shm buffer
        memcpy(&shm_pointer[index_of_read_index], &read_index, 4);		// write the read_index value to the shm buffer
    }
    else
    {
        memcpy(&write_index, &shm_pointer[index_of_write_index], 4);	// read the write_index value from the shm buffer
        memcpy(&read_index, &shm_pointer[index_of_read_index], 4);		// read the read_index value from the shm buffer
    }
}
...
void EmuShm::write32(uint32_t word)
{
    ...
    // do the write
    memcpy(&shm_pointer[write_index], &word, 4);
...
uint32_t EmuShm::read32()
{
    ...
    // do the read
    memcpy(&word, &shm_pointer[read_index], 4);
...
{
    // Update values
    memcpy(&write_index, &shm_pointer[index_of_write_index], 4);
    memcpy(&read_index, &shm_pointer[index_of_read_index], 4);
...
uint32_t EmuShm::getCurSize() {
    memcpy(&write_index, &shm_pointer[index_of_write_index], 4);
    memcpy(&read_index, &shm_pointer[index_of_read_index], 4);
    return ((write_index - read_index)+(shm_size-2*4))%(shm_size-2*4);
}
