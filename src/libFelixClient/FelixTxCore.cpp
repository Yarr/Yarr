...
void FelixTxCore::prepareTrigger(std::vector<uint8_t>& trigFifo) {
  ...
  case ITK_Pixel: //For ITk pixel RM 5.0 firmware
    if(m_pixFwTrigger){ //FW-based triggers with special 16b character in the F/W = {1110, #iteration (7b), frequency(5b)
      ...
      if(trigFreq_ratio > 31) { ftlog->error("m_trigFreq {} not supported by the F/W. Supported frequency is >= 9.8 kHz", m_trigFreq); exit(1);}
      ...
      double trigFreq_FW = (40000000/128)*1/(trigFreq_ratio+1); //trigger frequency as generated by FELIX firmware, for the given trigger frequency in YARR scan config
      static int count=0;
      if(count==0){
