...
void FelixTxCore::prepareTrigger(std::vector<uint8_t>& trigFifo) {
  ...
  case ITK_Pixel: //For ITk pixel RM 5.0 firmware
    if(m_pixFwTrigger){ //FW-based triggers with special 16b character in the F/W = {1110, #iteration (7b), frequency(5b)
      uint32_t trigFreq_ratio = (40000000/m_trigFreq)/128; //40 MHz/m_trigFreq(Hz) and /128(clocks) as F/W can in/decrease frequency only in multiple of 128(clocks)
      trigFreq_ratio = trigFreq_ratio-1; //compensating for the trigger pattern being 256 clocks long

      if(trigFreq_ratio > 31) { ftlog->error("m_trigFreq {} Hz not supported by the F/W. Supported frequency is >= 9.8 kHz", m_trigFreq); exit(1);}
      if(trigFreq_ratio == 0) { ftlog->error("m_trigFreq {} Hz not supported by the F/W. Supported frequency is <~ 156 kHz", m_trigFreq); exit(1);}
      if(m_trigCnt > 127)     { ftlog->error("m_trigCnt {} Hz not supported by the F/W. Supported range is 1 to 127", m_trigFreq); exit(1);}
