...
auto logger = logging::make_log("Itkpixv2ConnectivityScan");
...
void printHelp() {
    std::cout << "Usage: ./bin/connectivityScanner [-h] [-r <hw_controller_file>] [-c <connectivity_file>] [-o <output_path>]\n\n"
	  ...
	  << "  -c         <connectivity_file>    Specify connectivity config directory or JSON path. Default is \"configs/connectivity/auto_Itkpixv2_setup.json\"\n"
...
int main(int argc, char **argv) {
    ...
    j["log_config"][1]["name"] = "Itkpixv2GlobalCfg";
    j["log_config"][1]["level"] = "critical";
    j["log_config"][2]["name"] = "Itkpixv2PixelCfg";
    j["log_config"][2]["level"] = "critical";
    //j["log_config"][3]["name"] = "Itkpixv2ConnectivityScan";
    ...
    std::string connectivity_filename = "configs/connectivity/auto_Itkpixv2_setup.json";
	...
	for (int _rx = 0; _rx < nrx; _rx++) {
	    ...
	    // assuming Itkpixv2 quads (can be made more generic for triplets?) and 1.28GHz
	    json cfg;
	    cfg["ITKPIXV2"]["Parameter"]["ChipId"] = 16; // set chip ID to 16 to broadcast
	    // lane setting needed hmmmmmmmmm
	    cfg["ITKPIXV2"]["GlobalConfig"]["AuroraActiveLanes"] = (1 << nlanes)-1; //aurora active lanes = (2^nlanes)-1
	    ...
	    cfg["ITKPIXV2"]["Parameter"]["Name"] = chip_name.str();
