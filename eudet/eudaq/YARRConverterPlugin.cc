...
This code is adapted from the RCE version: https://github.com/eudaq/eudaq/blob/master/producers/apixct/src/APIX-CT-ConverterPlugin.cc.
...
namespace eudaq {
  ...
  private:
    ...
    YARRConverterPlugin() : DataConverterPlugin("APIX-CT"), m_nFrames(1), \
    ...
    static YARRConverterPlugin const m_instance;
  };
  
#if USE_LCIO && USE_EUTELESCOPE
  void YARRConverterPlugin::ConvertLCIOHeader(lcio::LCRunHeader & header, eudaq::Event const & , eudaq::Configuration const & ) const
  {
    eutelescope::EUTelRunHeaderImpl runHeader(&header);
  }
  
  
  bool YARRConverterPlugin::GetLCIOSubEvent(lcio::LCEvent & lcioEvent, const Event & eudaqEvent) const
  {
  ...
  YARRConverterPlugin const YARRConverterPlugin::m_instance;

  void YARRConverterPlugin::Initialize(const Event & source, const Configuration &) {
  ...
  bool YARRConverterPlugin::GetStandardSubEvent(StandardEvent & result, const Event & source) const {
    if (source.IsBORE()) {
      // shouldn't happen
      return true;
  ...
  unsigned YARRConverterPlugin::GetTriggerID(eudaq::Event const & ev) const {
  ...
  void YARRConverterPlugin::ConvertPlanes(const RawDataEvent & ev, std::map<int, StandardPlane>& result) const {
