...
if "${reset}"; then
    ...
    echo -e "      -> remove retrieve repository in ${HOME}/.localdb_retrieve"
    echo -e "Continue? [y/n]"
    read -p "> " answer
    while [ -z ${answer} ]; 
    do
...
echo -e ""
echo -e "[LDB]  - Check pip packages: '${setting_dir}/requirements-pip.txt'"
echo -e "[LDB]  - Create Local DB files:"
echo -e "[LDB]         - ${dir}"
echo -e "[LDB]         - ${dir}/log"
echo -e "[LDB]         - ${dir}/database.json"
...
echo -e "### $ITSNAME" | tee -a ${readme}
echo -e "- 'YARR/localdb/bin/localdbtool-upload --scan <path to result directory>' can upload scan data" | tee -a ${readme}
echo -e "- 'YARR/localdb/bin/localdbtool-upload --dcs <path to result directory>' can upload dcs data based on scan data" | tee -a ${readme}
echo -e "- 'YARR/localdb/bin/localdbtool-upload --cache' can upload every cache data" | tee -a ${readme}
echo -e "- 'YARR/localdb/bin/localdbtool-upload --help' can show more usage." | tee -a ${readme}
echo -e "" | tee -a ${readme}

# retrieve.py
ITSNAME="LocalDB Tool Setup Retrieve Tool"
echo -e "### $ITSNAME" | tee -a ${readme}
echo -e "- 'YARR/localdb/bin/localdbtool-retrieve init' can initialize retrieve repository" | tee -a ${readme}
echo -e "- 'YARR/localdb/bin/localdbtool-retrieve remote add <remote name>' can add remote repository for Local DB/Master Server" | tee -a ${readme}
echo -e "- 'YARR/localdb/bin/localdbtool-retrieve --help' can show more usage." | tee -a ${readme}
echo -e "" | tee -a ${readme}
...
echo -e "   - './localdb/bin/localdbtool-upload --cache' can upload every cache data" | tee -a ${readme}
echo -e "3. Retrieve function" | tee -a ${readme}
echo -e "   - './localdb/bin/localdbtool-retrieve log' can show test data log in Local DB" | tee -a ${readme}
echo -e "   - './localdb/bin/localdbtool-retrieve checkout <module name>' can restore the latest config files from Local DB" | tee -a ${readme}
