...
Developers shall commit their code to a new branch, which is feature-oriented i.e. that different features should be developed in separate branches. Every branch should have an open Issue or Merge Request once it's pushed to the main repository.
...
Static analysis is run in CI via CodeChecker (clang-tidy); see `scripts/run_codechecker.sh` for the list of enabled checks.
